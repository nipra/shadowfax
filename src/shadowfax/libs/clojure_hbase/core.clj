(ns shadowfax.libs.clojure-hbase.core
  (:refer-clojure :rename {get map-get})
  (:use shadowfax.libs.clojure-hbase.internal)
  (:require [shadowfax.libs.clojure-hbase [util :as hu]])
...
(defn- make-get
  ...
  (let [row (to-bytes row)
    ...
    (cond (contains? cons-opts :use-existing)
...
(defn- handle-put-values
  ...
  (doseq [value (partition 2 values)]
    ...
    (let [[family params-list] value]
      (apply put-add put-op family params-list)
  ...
  (let [specs   (partition-query args scan-argnums)
    ...
    (doseq [spec specs]
      (condp = (first spec)
          ...
          :caching      (.setCaching scan-op (second spec))))
    scan-op))
