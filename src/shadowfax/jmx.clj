...
(def replication-source-metrics [:ageOfLastShippedOp
...
(def replication-sink-metrics [:ageOfLastAppliedOp
                               :appliedBatchesRate
                               :appliedOpsRate])
...
;;; 
(defn get-replication-source-metrics*
  [regionserver metrics peer-id]
  (jmx/with-connection {:host regionserver :port hbase-regionserver-jmx-port}
    (metrics (jmx/mbean (str "hadoop:service=Replication,name=ReplicationSource for " peer-id)))))

(defn get-replication-source-metrics
  [metrics peer-id]
  (let [regionservers (get-regionservers)
        metrics* (map #(reduce (fn [x y]
                                 (merge x {y (get-replication-source-metrics* % y peer-id)}))
                               {}
                               metrics)
                      regionservers)
        metrics** (map (fn [x y] (merge {:host x} y)) regionservers metrics*)]
...
;;;
(defn get-replication-sink-metrics*
  [regionserver metrics]
  (jmx/with-connection {:host regionserver :port hbase-regionserver-jmx-port}
    (metrics (jmx/mbean "hadoop:service=Replication,name=ReplicationSink"))))

(defn get-replication-sink-metrics
  [metrics]
  (let [regionservers (get-regionservers)
        metrics* (map #(reduce (fn [x y]
                                 (merge x {y (get-replication-sink-metrics* % y)}))
                               {}
                               metrics)
                      regionservers)
        metrics** (map (fn [x y] (merge {:host x} y)) regionservers metrics*)]
